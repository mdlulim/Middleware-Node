...
export const uploadSelfie: MutationResolvers['uploadSelfie'] = async function (
  _parent: any,
  ...
  try {
    ...
    console.log('UploadSignature registerStatus:', registerStatus);
    ...
    if (applicationStatus) {
      (Object.keys(
        ...
        if (applicationStatus[key].Name === 'Image_Selfie') {
          if (applicationStatus[key].Status) {
            return {success: true, message: 'Success', status: 3};
          }
        }
      });
    }
    ...
    // fs.writeFile('register_selfie.jpg', base64Image, 'base64', function(err) {
    //   if (err) {return console.log(err);}
    //   console.log('register_selfie.jpg created');
    // });

    const image: DigitalOnboardingImage = {
      _type: DigitalOnboardingImageType.Selfie,
      Base64encoded_file: base64Image,
      filename: 'selfie.jpg',
    };
    const uploadSelfieResult = await uploadImage(context, mendixAppId, image);
    console.log({uploadSelfieResult});

    if (!uploadSelfieResult) {
      return {
        success: false,
        message: 'Could not upload selfie',
        status: 1,
      };
    }
    ...
    return {success: true, message: 'Success', status: 2};
  } catch (ex) {
    console.log(ex);
    return {success: false, message: ex.message, status: 1};
  }
};
