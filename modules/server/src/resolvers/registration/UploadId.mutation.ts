...
export const uploadId: MutationResolvers['uploadId'] = async function (
  _parent: any,
  ...
  try {
    ...
    // console.log(registerStatus.data[registerStatus.data.length - 1].reference)
    ...
    /**
     * check application status for ID pic
     */
    ...
    if (applicationStatus) {
      (Object.keys(
        ...
        if (applicationStatus[key].Name === 'Image_ID_Front') {
    ...
    // fs.writeFile('register_id.jpg', base64Image, 'base64', function(err) {
    //   if (err) {return console.log(err);}
    //   console.log('register_id.jpg created');
    // });
    const image: DigitalOnboardingImage = {
      _type: DigitalOnboardingImageType.IdFront,
      Base64encoded_file: base64Image,
      filename: 'id.jpg',
    };
    const uploadIdResult = await uploadImage(context, mendixAppId, image);
    // console.log({uploadSelfieResult: uploadIdResult})
    if (!uploadIdResult) {
      return {
        success: false,
        message: 'Could not upload ID',
        status: 2,
      };
    }
    ...
    console.log(ex);
    return {success: false, message: ex.message, status: 2};
  }
};
